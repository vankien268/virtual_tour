...
class Helper
{
     ...
     * Số Thứ Tự
     */
    public static function getSerial($page, $limit, $key){
        $serial = ($page - 1) * $limit + $key;
        return $serial;
    }
    // $serial = \Longtt\User\Helper::getSerial($records->currentPage(),$records->perPage(),$key+1)
    
    /**
     * đếm số ngôn ngữ tạo thêm bài thuyết trình
     *
     * @param  mixed $location
     * @return void
     */
    public static function countLanguage($location)
    {
        $languageIds = $location->presentations()->pluck('language_id');
        $languages = Language::whereNotIn('id', $languageIds)->where('status', 1)->count();
        return $languages;
    }
}
